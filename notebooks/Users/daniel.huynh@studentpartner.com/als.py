...
df.select("movieId").map(lambda x : movie_to_emb[x])
...
# We create a Dataframe to get the cumulative sums
...
# We create a dictionary which maps user Id to his cumulative sum of movies
usr_cum_count = usr_to_count.rdd.map(lambda x : x["cum_count"]).collect()

# We define a function to map user Id to a partition
