...
len(emb_to_movie), len(emb_to_usr)
...
t = df.select("userId").collect()

# COMMAND ----------

v = [x["userId"] for x in t]

# COMMAND ----------

sorted(usr_to_emb.values())
...
for l in range(N_EPOCH):
  ...
  y = df.rdd.map(lambda x : ( (x["userId"],x["movieId"]), x["rating"]) )
